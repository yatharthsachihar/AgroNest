...
export default function CheckoutPage() {
  ...
  // B2B / enquiry mode has no checkout — send shoppers back home.
  useEffect(() => {
    if (settings?.storeMode === "b2b") navigate("/", { replace: true });
  }, [settings?.storeMode, navigate]);

  // Authentication Gate (skipped in B2B — the effect above already redirects)
  useEffect(() => {
    if (settings?.storeMode === "b2b") return;
    if (!userLoading && !user) {
      toast.error("Please login to proceed to checkout.");
      navigate("/login", { state: { from: "/checkout" } });
    }
  }, [user, userLoading, navigate, settings?.storeMode]);
