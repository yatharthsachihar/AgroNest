...
export const UserProvider = ({ children }) => {
  ...
  useEffect(() => {
    const token = localStorage.getItem('axiomcropsciences_user_token');
    if (!token) { setLoading(false); return; }
    userAuthApi.me()
      .then(res => {
        if (res.data && res.data.isActive === false) {
          localStorage.removeItem('axiomcropsciences_user_token');
          setUser(null);
  ...
  useEffect(() => {
    const handleAuthError = () => {
      if (!localStorage.getItem('axiomcropsciences_user_token')) return;
      logout();
      navigate('/deactivated', { replace: true });
    };
  ...
  const login = async (identifier, password) => {
    ...
    localStorage.setItem('axiomcropsciences_user_token', res.data.token);
